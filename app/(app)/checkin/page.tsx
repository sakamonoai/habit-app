...
export default async function CheckinPage() {
  ...
  const groups = memberships.map(m => {
    ...
    const notStartedYet = challenge?.schedule_type === 'fixed' && challenge.start_date
      ...
      : (m.joined_at ? new Date(m.joined_at) > now : false)
    const startDateLabel = challenge?.schedule_type === 'fixed' && challenge.start_date
      ? challenge.start_date
      : m.joined_at ? m.joined_at.split('T')[0] : null

    // 達成判定
    const isAchieved = checkinCount >= requiredDays
    const isPerfect = checkinCount >= durationDays
  ...
  // 期間終了かつ達成したチャレンジ（ポップアップ用）
  const completedChallenges = groups
    .filter(g => !g.isOngoing && g.isAchieved)
    .map(g => ({ title: g.title, isPerfect: g.isPerfect, rate: g.rate }))

  return (
    <div className="min-h-screen bg-white">
      {completedChallenges.length > 0 && (
        <ChallengeCompletePopup challenges={completedChallenges} />
      )}
      <header className="sticky top-0 z-10 bg-white">
        <div className="max-w-lg mx-auto px-4 pt-4 pb-2">
          <h1 className="text-2xl font-bold text-gray-900">記録</h1>
              ...
              >
                ...
                {g.isPerfect ? (
                  <div className="mt-2 bg-green-50 border border-green-200 rounded-lg px-3 py-2">
                    <p className="text-xs text-green-700 font-semibold">🎉 パーフェクト達成！すべての日をクリアしました！</p>
                  </div>
                ) : g.isAchieved ? (
                  <div className="mt-2 bg-green-50 border border-green-200 rounded-lg px-3 py-2">
                    <p className="text-xs text-green-700 font-semibold">✨ 達成確定！おめでとうございます！</p>
                  </div>
